...
public class DoorReversalMonitor extends RuntimeMonitor{
    ...
    private void doorNudging(Hallway hallway) {
        ...
        if (!doorReversalTriggeredBefore) {
            warning("R-T.10 Violated: Door started nudging before at least one door reversal occured.");
        }
    }
    ...
    private void doorClosed(Hallway hallway) {
        ...
        if (!doorState.anyDoorOpen()) {
        	watch.stop();
            doorReversalTriggeredBefore = false;
            if (wasOverweight) {
                message("Overweight");
                overWeightCount++;
                wasOverweight = false;
    ...
    public void receive(ReadableDoorReversalPayload msg) {
        if (doorState.anyDoorMotorClosing(msg.getHallway())) {
